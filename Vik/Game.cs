...
namespace Vik
{
    ...
    {
        private FossilsEat EatResources = new FossilsEat();
        private FossilsForest ForestResources = new FossilsForest();
        private FossilsStone StoneResources = new FossilsStone();
        private FossilsGold GoldResources = new FossilsGold();
        private ResourceVillage Village = ResourceVillage.GetInstance();
        private Buildheadquarters Headquarters = new Buildheadquarters();
        private BuildPort Port = new BuildPort();
        private BuildGoldRec GoldRec = new BuildGoldRec();
        private BuildStoneRec StoneRec = new BuildStoneRec();
        private BuildWoodRec WoodRec = new BuildWoodRec();
        private Buildmarket Market = new Buildmarket();
        private List<Buildgoldmine> Goldmine = new List<Buildgoldmine>(10);
        private List<Buildquarry> Quarry = new List<Buildquarry>(10);
        private List<Buildsawmill> Sawmill = new List<Buildsawmill>(10);
        private List<Buildstorage> Storage = new List<Buildstorage>(10);
        private List<Buildfield> Field = new List<Buildfield>(10);
        private List<BuildWall> Wall = new List<BuildWall>(50);
        private List<Buildhome> Home = new List<Buildhome>(10);
        private List<BuildSanctuary> Sanctuary = new List<BuildSanctuary>(10);
        private List<BuildBarn> Barn = new List<BuildBarn>(10);
        private List<BuildBarracks> Barracks = new List<BuildBarracks>(10);
        private List<Enemy> Bandits = new List<Enemy>(20);
        ...
        private void b_Click(object sender, EventArgs e)
        {
            ...
            if (BuildMap[p.X, p.Y] == 0)
            {
                ...
                if (Village.GetChoose() == 13 && Village.GetBrick() >= 5) // стена
                {
					/*
                        EnemySpawn();
                        Thread enemythread = new Thread(EnemyPriority);
                        enemythread.Start();
					*/
        ...
        private void EnemySpawn() // противников противников
        {
            ...
            if(check > 0)
            {
                pointSpawn = rd.Next(0, check - 1);
                Enemy enemy = new Enemy(Convert.ToByte(arrX[pointSpawn]), Convert.ToByte(arrY[pointSpawn]), 50, 1);
                Bandits.Add(enemy);
            }
        }
        ...
        private void EnemyPriority() // поток раставляющий приоритет противников
        {
            Random rd = new Random();
            int[] x = new int[Bandits.Count];
            int[] y = new int[Bandits.Count];
            int i = 0;
            int checkUpY = 0;
            int checkLeftX = 0;
            int checkDownY = 0;
            ...
            while (Village.GetPopulation() > 0)
            {
                foreach (Enemy obj in Bandits)
                {
                    ...
                    do
                    {
                        ...
                        {
                            if (BuildMap[x[i], y[i]] >= 10 && BuildMap[x[i], y[i]] <= 26) break;
                            if (x[i] - 1 >= 0 && checkLeftX != -1)
                            {
                                x[i] = x[i] - 1;
                                checkLeftX = x[i];

                                obj.SetX(Convert.ToByte(x[i]));
                                obj.SetY(Convert.ToByte(y[i]));
                                obj.SetImage();
                        ...
                        if (BuildMap[x[i], y[i]] >= 10 && BuildMap[x[i], y[i]] <= 26) break;
                        minus++;
                    } while (BuildMap[x[i], y[i]] <= 10 && BuildMap[x[i], y[i]] >= 26);
                    if (minus != -1)
                    {
                        obj.SetImage();
                        int p = 0;
                        for (int u = 0; u < p + 1; u++)
                        {
                            ...
                            obj.SetDeleteImage();
                            if (obj.GetY() > y[i])
                                obj.SetY(Convert.ToByte(obj.GetY() - 1));
                            else if (obj.GetY() < y[i])
                                obj.SetY(Convert.ToByte(obj.GetY() + 1));
                            if (obj.GetX() > x[i])
                                obj.SetX(Convert.ToByte(obj.GetX() - 1));
                            else if (obj.GetX() < x[i])
                                obj.SetX(Convert.ToByte(obj.GetX() + 1));
                            if ((obj.GetY() == y[i] - 1 && obj.GetX() == x[i] - 1)
                                || (obj.GetY() == y[i] && obj.GetX() == x[i] - 1)
                                || (obj.GetY() == y[i] + 1 && obj.GetX() == x[i] - 1)
                                || (obj.GetY() == y[i] + 1 && obj.GetX() == x[i])
                                || (obj.GetY() == y[i] + 1  && obj.GetX() == x[i] + 1)
                                || (obj.GetY() == y[i] && obj.GetX() == x[i] + 1)
                                || (obj.GetY() == y[i] - 1 && obj.GetX() == x[i] + 1)
                                || (obj.GetY() == y[i] + 1 && obj.GetX() == x[i]))
                        ...
                        switch (BuildMap[i, j])
                        {
                            ...
                            case 17:
                                foreach (BuildWall k in Wall)
                                    if (k.GetX() == i && k.GetY() == j)
                                    {
                                        Action action = () => t.SetToolTip(arr[i, j], "Стена\nСостояние: " + k.GetHealth());
                                        if (InvokeRequired)
                                            Invoke(action);
                                        else
                                            action();
        ...
        private void Spoiled() // поток испорченности еды
        {
            ...
            while (Village.GetPopulation() > 0)
            {
                ...
                if (chance > rd.Next(10,30))
                {
                    ...
                    if (Village.GetEat() - chance >= 0) Village.SetEat(Convert.ToUInt16(Village.GetEat() - chance));
        ...
        private void OtherReligion() // поток сравнивающий религию деревни и конунга
        {
            while (Village.GetPopulation() > 0)
            {
				...
				if (Village.GetReligionKonung() != Village.GetReligion())
					if (Village.GetReligionStatic() - 15 + Sanctuary.Count >= -100) Village.SetReligionStatic(Convert.ToSByte(Village.GetReligionStatic() - 15 + Sanctuary.Count));
					else Village.SetReligionStatic(0);
			}
        }

        private void BuildingAging() // поток состаривания зданий
        {
            while (Village.GetPopulation() > 0)
            {
                Thread.Sleep(1000);
                ...
                if (Home.Count > 0)
                    foreach (Buildhome i in Home)
                    {
                        ...
                        {
                            i.SetDeleteImage();
							if (Village.GetPopulationMax() - 15 >= 0) Village.SetPopulationMax(Convert.ToUInt16(Village.GetPopulationMax() - 15));
							else Village.SetPopulationMax(0);
							BuildMap[i.GetX(), i.GetY()] = 0;
                            Home.Remove(i);
                            break;
                        }
                ...
                if (Storage.Count > 0)
                    foreach (Buildstorage i in Storage)
                    {
                        ...
                        {
							if (Village.GetEatMax() - 30 >= 0) Village.SetEatMax(Convert.ToUInt16(Village.GetEatMax() - 30));
							else Village.SetEatMax(0);
							if (Village.GetForestMax() - 10 >= 0) Village.SetForestMax(Convert.ToUInt16(Village.GetForestMax() - 10));
							else Village.SetForestMax(0);
							if (Village.GetStoneMax() - 10 >= 0) Village.SetStoneMax(Convert.ToUInt16(Village.GetStoneMax() - 10));
							else Village.SetStoneMax(0);
							if (Village.GetGoldMax() - 5 >= 0) Village.SetGoldMax(Convert.ToUInt16(Village.GetGoldMax() - 5));
							else Village.SetGoldMax(0);
							if (Village.GetBoardsMax() - 20 >= 0) Village.SetBoardsMax(Convert.ToUInt16(Village.GetBoardsMax() - 20));
							else Village.SetBoardsMax(0);
							if (Village.GetBrickMax() - 20 >= 0) Village.SetBrickMax(Convert.ToUInt16(Village.GetBrickMax() - 20));
							else Village.SetBrickMax(0);
							Action action = () => label3.Text = Village.GetForest().ToString();
							if (InvokeRequired)
								Invoke(action);
							else
								action();
							action = () => label4.Text = Village.GetStone().ToString();
							if (InvokeRequired)
								Invoke(action);
							else
								action();
							action = () => label11.Text = Village.GetEatMax().ToString();
							if (InvokeRequired)
								Invoke(action);
							else
								action();
							action = () => label12.Text = Village.GetForestMax().ToString();
							if (InvokeRequired)
								Invoke(action);
							else
								action();
							action = () => label13.Text = Village.GetStoneMax().ToString();
							if (InvokeRequired)
								Invoke(action);
							else
								action();
							action = () => label14.Text = Village.GetGoldMax().ToString();
							if (InvokeRequired)
								Invoke(action);
							else
								action();
							action = () => label9.Text = Village.GetBoardsMax().ToString();
							if (InvokeRequired)
								Invoke(action);
							else
								action();
							action = () => label10.Text = Village.GetBrickMax().ToString();
							if (InvokeRequired)
								Invoke(action);
							else
								action();
